  ...
  if (isMobile) {
    return (
      <div className="px-5 pt-6 pb-10">
        <h1 className="font-['JetBrains_Mono'] text-[#141414] font-bold text-[32px] leading-[1.05] tracking-[-0.02em] mb-2">
          {projectName}
        </h1>
        <p className="font-['JetBrains_Mono'] text-[12px] text-[#141414] opacity-40 leading-[1.5] mb-5 max-w-[400px]">
          {subtitle}
        </p>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                to={item.path}
                className={`font-['JetBrains_Mono'] text-[14px] font-bold no-underline transition-opacity duration-300 hover:opacity-60 flex items-center gap-2 ${
                  isActive ? 'text-[#141414]' : 'text-[#141414] opacity-30'
                }`}
              >
                {isActive && (
                  <span className="w-3.5 h-3 shrink-0 text-[#141414]"><PlayIcon /></span>
                )}
                {item.label}
              </Link>
            );
        ...
        <div className="w-full min-h-[40vh] mb-8">
          {children || (
            <span className="text-[#141414] text-[14px] opacity-20 tracking-[4px] font-['JetBrains_Mono'] uppercase">
              {activeItem}
            </span>
          )}
        </div>
  ...
  return (
    <>
      <h1
        ...
        style={{ left: 58, top: 95, fontSize: 56, letterSpacing: '-0.02em' }}
      >
        {projectName}
      </h1>

      <p
        className="absolute font-['JetBrains_Mono'] text-[#141414] opacity-40 leading-[1.5]"
        style={{ left: 60, top: 168, fontSize: 12, maxWidth: 550, letterSpacing: '0em' }}
      >
        {subtitle}
      </p>

      {/* Horizontal divider between title section and content */}
      <div className="absolute bg-[#D2D2D2]" style={{ left: 58, right: 58, top: 220, height: 1 }} />

      {menuItems.map((item, index) => {
        const isActive = item.label === activeItem;
        const top = menuStartY + index * menuGap;
        const left = isActive ? 88 : 58;

        return (
          <Link
            key={item.label}
            to={item.path}
            className={`absolute text-[18px] font-bold leading-[1] no-underline transition-all duration-300 hover:opacity-60 font-['JetBrains_Mono'] ${
              isActive ? 'text-[#141414]' : 'text-[#141414] opacity-30'
            }`}
            style={{ left, top }}
          >
      ...
      {menuItems.map((item, index) => {
        ...
        return (
          <div key="arrow" className="absolute w-[14px] h-[12px] text-[#141414]" style={{ left: 58, top }}>
            <PlayIcon />
          </div>
        );
      })}
      ...
      {applications && applications.length > 0 && (
        <div className="absolute" style={{ left: 58, bottom: 60 }}>
      ...
      <div
        ...
        style={{ left: 340, top: 280, width: 850, height: 700 }}
      >
        {children || (
...
export function SelectionLogicOverviewPage() {
  return (
    <LabProjectPage projectName="SELECTION LOGIC" subtitle={SL_SUBTITLE} menuItems={selectionLogicMenu} activeItem="overview" applications={selectionLogicApps}>
      <div className="pt-2 max-w-[700px]">
        <p className="font-['JetBrains_Mono'] text-[16px] text-[#141414] opacity-70 leading-[1.9] mb-10">
          Selection Logic determines how a system chooses one outcome among many possible configurations.
        </p>
        <p className="font-['JetBrains_Mono'] text-[13px] text-[#141414] opacity-35 leading-[1.9]">
      ...
      <div className="pt-2 flex flex-col gap-5">
        {principles.map((p, i) => (
          <div key={p} className="flex items-center gap-5">
            <span className="font-['JetBrains_Mono'] text-[40px] font-bold text-[#141414] opacity-10 w-[50px] text-right">{String(i + 1).padStart(2, '0')}</span>
            <span className="font-['JetBrains_Mono'] text-[20px] font-bold text-[#141414] opacity-70 tracking-[2px]">{p}</span>
          </div>
        ))}
      </div>
    </LabProjectPage>
...
export function SelectionLogicDiagramPage() {
  return (
    <LabProjectPage projectName="SELECTION LOGIC" subtitle={SL_SUBTITLE} menuItems={selectionLogicMenu} activeItem="diagram" applications={selectionLogicApps}>
      <div className="pt-2 flex flex-col items-center justify-center h-full gap-10">
...
export function DenseFieldDiagramPage() {
  return (
    <LabProjectPage projectName="DENSE FIELD" subtitle={DF_SUBTITLE} menuItems={denseFieldMenu} activeItem="diagram" applications={denseFieldApps}>
      <div className="pt-2 flex items-center justify-center h-full gap-20">
...
export function ModularFillDiagramPage() {
  return (
    <LabProjectPage projectName="MODULAR FILL" subtitle={MF_SUBTITLE} menuItems={modularFillMenu} activeItem="diagram" applications={modularFillApps}>
      <div className="pt-2 flex flex-col items-center justify-center h-full gap-8">
...
export function LayerStackDiagramPage() {
  return (
    <LabProjectPage projectName="LAYER STACK" subtitle={LS_SUBTITLE} menuItems={layerStackMenu} activeItem="diagram" applications={layerStackApps}>
      <div className="pt-2 flex items-center justify-center h-full">
